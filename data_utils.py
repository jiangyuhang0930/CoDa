...
def create_trajectory_dataset(env, num_trajectory=10000, num_transition=20, reset_prob=0.5, print_every=1000):
  train = []
  sprites = []
  s1 = env.reset()
  if hasattr(env, '_env'):
  ...
  sprites1 = copy.deepcopy(_env.state()['sprites'])
  i = 1
  while len(train) < num_trajectory:
    trajectory = []
    trajectory_sprites = []
    i += 1
    if i % print_every == 0:
      print('.', end='', flush=True)
    while len(trajectory) < num_transition:
      a = env.action_space.sample()
      s2, r, _, _ = env.step(a)
      a = np.concatenate(([0, 0], a), None).reshape((1, 4))
      trajectory.append(np.concatenate((s1['disentangled'], a), axis=0))
      trajectory_sprites.append(sprites1)
  
      s1 = s2
      sprites1 = copy.deepcopy(_env.state()['sprites'])
    train.append(trajectory)
    sprites.append(trajectory_sprites)
    if np.random.random() < reset_prob:
      s1 = env.reset()
      sprites1 = copy.deepcopy(_env.state()['sprites'])
  return train, sprites
