...
from nri_utils import my_softmax, get_offdiag_indices, gumbel_softmax, mask_transform_fetch, mask_transform_back
...
class NRI(nn.Module):
    ...
    def forward_with_mask(self, x, rel_rec, rel_send, indices, thresh):
        logits = self.encoder(x, rel_rec, rel_send)
        edges, copy = gumbel_softmax(logits, tau=0.5, hard=True, thresh=thresh)
        pred = self.decoder(x, edges, rel_rec, rel_send, indices)
        return pred, mask_transform_fetch(copy, 2)
