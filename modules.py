...
class SingleStepDecoder(nn.Module):
  ...
  def forward(self, inputs, rel_type, rel_rec, rel_send):
    pred = self.single_step_forward(inputs, rel_rec, rel_send, rel_type)
    return pred[:, :-1, :].contiguous()

  def forward_prev(self, inputs, rel_type, rel_rec, rel_send, pred_steps=1):
      # NOTE: Assumes that we have the same graph across all samples.
...
class MultiStepDecoder(nn.Module):
  ...
  def forward(self, encoder, inputs, rel_rec, rel_send, pred_steps):
    inputs = inputs[:, 0, :, :].squeeze(1)
    preds = []
    probs = []
    output_pred = torch.zeros(inputs.size(0), pred_steps, inputs.size(1), inputs.size(2))
    output_prob = torch.zeros(inputs.size(0), pred_steps, 25, 2)
    for i in range(pred_steps):
        logits = encoder(inputs, rel_rec, rel_send)
        edges = gumbel_softmax(logits, tau=0.5, hard=True)
        pred = self.single_step_forward(inputs, rel_rec, rel_send, edges)
        preds.append(pred)
        probs.append(my_softmax(logits, -1))
        inputs = pred
    for i in range(pred_steps):
        output_pred[:, i, :, :] = preds[i]
        output_prob[:, i, :, :] = probs[i]
...
class MLPDecoder(nn.Module):
    ...
    def __init__(self, encoder, n_in_node, edge_types, msg_hid, msg_out, n_hid,
                 do_prob=0., skip_first=False):
    ...
    def forward(self, inputs, encoder, rel_rec, rel_send, pred_steps=1):
        # NOTE: Assumes that we have the same graph across all samples.
        ...
        sizes = [rel_type.size(0), inputs.size(1), rel_type.size(1),
                 rel_type.size(2)]
        # rel_type = encoder(inputs, rel_rec, rel_send)
        # rel_type = rel_type.unsqueeze(1).expand(sizes)
